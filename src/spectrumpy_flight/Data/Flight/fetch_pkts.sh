...
if [[ ${#remote_files[@]} -eq 0 ]]; then
  echo "No matching l0 raw packet files found." >&2
  exit 1
fi
...
shopt -s nullglob
for local_file in "$SCRIPT_DIR"/imap_idex_l0_raw_*.pkts; do
...
echo "Fetch complete. Current latest packet files are in $SCRIPT_DIR"
