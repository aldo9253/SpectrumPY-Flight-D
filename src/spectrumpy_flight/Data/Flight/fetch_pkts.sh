...
if [[ ${#remote_files[@]} -eq 0 ]]; then
  echo "No matching $DATA_LEVEL $DESCRIPTOR .$EXTENSION files found." >&2
  exit 1
fi

parse_product_name() {
  local file_name="$1"
  if [[ "$file_name" =~ ^(.+)_v([0-9]+)\.${EXTENSION}$ ]]; then
...
for remote_path in "${remote_files[@]}"; do
  ...
  if ! read -r remote_stem remote_version < <(parse_product_name "$base_name"); then
    echo "Skipping unrecognized remote packet name: $base_name" >&2
    continue
  fi
  ...
  shopt -s nullglob
  for local_file in "$SCRIPT_DIR/$remote_stem"_v*."$EXTENSION"; do
    local_base="$(basename "$local_file")"
    if ! read -r local_stem local_version < <(parse_product_name "$local_base"); then
      continue
    fi
...
echo "Fetch complete. Current latest $DATA_LEVEL $DESCRIPTOR .$EXTENSION files are in $SCRIPT_DIR"
